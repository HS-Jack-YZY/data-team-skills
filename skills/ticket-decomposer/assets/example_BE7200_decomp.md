...
> **假设的需求方反馈**（已合并进 alignment_v2）：Q1=A（仅 3 决策点）·Q2=B（US+DE+UK）·Q3=A（HTML 报告）·Q4=B（Flint2 作上代用户参考组）·Q5=A（**选精准版**，约 5–6 工作日）。alignment_v2 第三章选定档：**精准版 5–6 工作日 ≈ 30–36h**（按 6h/工作日 换算）。本 Plan 反推工具级后总 h 应在此区间 ±10% 容差内。
...
- **1 人串行**：Σ(U1+U2+U3+U4+U5+U6+U7+U8+U9+U10) = 5+5+2+1+2+1.5+4+3+0.5+6 = **30h** ≈ 5 工作日
- **2 人并行（典型）**：关键路径 = max(U1, U2, U3, U4, U5) → U6 → U7 → max(U8, U9) → U10 = 5 + 1.5 + 4 + 3 + 6 = **19.5h** ≈ 3.3 工作日
- **3 人最大并行**：关键路径 ≈ 18h ≈ 3 工作日（U10 4 章可由 3 人写完）

**与 alignment_v2 一致性自检（Rev 2）**：

- alignment_v2 第三章选定档：**精准版 5–6 工作日**
- 换算成小时（6h/工作日）：**30–36h** 区间
- 本 Plan 总 h = **30h**
- 偏差判定：30h 落在 30–36h 区间下沿，进一步看与中位 33h 的偏差 = -9.1%（在 ±10% 容差内 ✅）

**为什么 B 反推后 h 数与 alignment_v2 工作日数能对得上**：

- B 用的 capabilities.md 工时基线 = ticket-aligner 计算工作日时所用的同一份基线（单源约定）
- 6h/工作日 是两 skill 的共用换算系数
...
如果偏差超过 ±10%，B **不得**自行抹平——停下来报错让用户检查：是 capabilities.md 不准？alignment_v2 工作日数偏差大？还是 alignment_v2 第二章描述漏了某项？
...
### M4：HTML 报告交付（≈ 第 3 工作日下午 / 第 4 工作日）
- 包含：U10
