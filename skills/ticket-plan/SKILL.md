...
> **完整定义见 [`references/path_conventions.md`](references/path_conventions.md)**：含 mermaid 流程图、目录树、`data/` 三类子目录（raw / analyzed / scratch）、文件命名规则、正确路径示例、6 类反模式、各 skill 角色映射。

**生成 plan 时只需记住三句话**（细节去看 path_conventions.md）：

1. **三层关注点**：`<root>/docs/`（人读过程材料）+ `<root>/data/`（机器分析数据）+ `<root>/<root>/`（对外交付包）。其中 `<root>` = `<编号>_<slug>/`
2. **数据落点**：plan 工作单元的「输出产物」字段，数据类一律 `<root>/data/raw/<source>/<descriptor>_<YYYYMMDD>.<ext>` 或 `<root>/data/analyzed/<descriptor>_<YYYYMMDD>.<ext>`
3. **报告落点**：HTML / PDF / xlsx 等对外交付物一律落 `<root>/<root>/`，而非 ticket 根

**最关键的反模式**（其余见 path_conventions.md § 6）：

- ❌ 把数据放到项目根 `data/raw/<source>/<编号>/` —— 必须在 ticket 单根 `<root>/data/...` 下
- ❌ HTML 落在 ticket 根 `<root>/<root>.html` —— 必须在交付包子目录 `<root>/<root>/<root>.html`
- ❌ plan.md 自身落到 ticket 根 / 交付包 —— plan.md 永远在 `<root>/docs/plan.md`（避免误发给需求方）
...
- ❌ 文件落到 `data/raw/<source>/<编号>/` 这种项目根 data/ 下 —— 必须在 `<编号>_<slug>/data/...`
...
> 解析参数 → 三模式判定（A 有 alignment / B 三要素齐 / C 反问 1–3 条最缺项）→ 读 capabilities + recipes + patterns → 按 5 / 7 章节骨架填，工时与 alignment 一致 ±10%（模式 A）或标 ±20% 未经对齐（模式 B / C）→ 依赖图用 mermaid 且无环 → 产物路径全在 `<编号>_<slug>/data/{raw/<source>,analyzed}/` 单根下 → 落盘 `<编号>_<slug>/docs/plan.md` → 在对话里给路径 + 工期摘要 + Top 3 风险。
