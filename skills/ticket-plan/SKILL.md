...
- **`docs/`** = 给人看的文档（过程材料、对齐稿、plan、交付消息归档）
- **`data/`** = 给机器/分析用的数据资产（原始抓取、清洗中间态、分析产物）
- **`<编号>_<slug>/`（同名子目录）** = 对外交付包（HTML 报告 + 附件 PDF / xlsx / assets/ / data 切片）—— 组员要交付时直接 zip 这个子目录即可，不会把 ticket 根的 `docs/` 和 `data/` 误打进去

**为什么 plan.md 在 `docs/` 下而不是 ticket 根**：`docs/` 是过程性的（组内消化用），ticket 根是结构骨架。把 plan.md 留在 docs/ 也意味着它不会出现在交付包子目录里——这是有意设计，组员不应该把内部 plan 发给需求方。
