...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/raw/amazon/amazon_reviews_<asin>_<market>_<YYYYMMDD>.csv`
...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/raw/reddit/reddit_<keyword>_<YYYYMMDD>.csv`（8 列标准字段）
- **预计耗时**：5h（10 关键词 × ~0.5h 实际，因每批 5 关键词，2 批跑批 + 清洗）
- **推荐工具**：Apify Reddit Scraper
- **基于 P-pattern**：P1
- **依赖**：无
- **可并行兄弟**：U1 / U3 / U4
- **DoD**：10 关键词全部跑完；每关键词清洗后行数 ≥ 80（冷门关键词 ≥ 30）；归档到 `DT-2026-04-28-01_BE7200-User-Research/data/raw/reddit/`
...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/raw/forum/glinet/forum_glinet_<keyword>_<YYYYMMDD>.csv`
- **预计耗时**：2h（4 关键词 × 0.5h）
- **推荐工具**：forum_scraper.py（Discourse API）
- **基于 P-pattern**：P3
- **依赖**：无
- **可并行兄弟**：U1 / U2 / U4
- **DoD**：4 搜索词跑完；归档到 `DT-2026-04-28-01_BE7200-User-Research/data/raw/forum/glinet/`
- **风险**：GL.iNet 论坛活跃用户偏极客，结论代表性偏 OpenWrt 用户群

### U4. OpenWrt 论坛抓取（2 搜索词）

- **输入**：搜索词 `desktop router hardware`, `usb tethering router`
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/raw/forum/openwrt/forum_openwrt_<keyword>_<YYYYMMDD>.csv`
- **预计耗时**：1h
...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/analyzed/dict_scenario_v1.json`（结构：home_office_words / travel_words / aliases）
...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/analyzed/labeled_<dataset>_<YYYYMMDD>.csv`（含 tag_id_design / tag_ports / tag_usb / scenario）
- **预计耗时**：1.5h
- **推荐工具**：Python 脚本（复用 clean_reddit_data.py 风格）
- **基于 P-pattern**：P8
- **依赖**：U1 + U2 + U3 + U4 + U5 全部完成
- **可并行兄弟**：无（汇聚节点）
- **DoD**：标注覆盖率 ≥ 60%；Flint2 数据单独 tag 为 `legacy_user`；归档到 `DT-2026-04-28-01_BE7200-User-Research/data/analyzed/`
- **风险**：MT6000 评论量大，标注耗时可能 +0.5h

### U7. LLM 语义分析（情感 + 主题聚类）

- **输入**：U6 标注后数据（预计总量 ~3000–4000 条）
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/analyzed/llm_<dataset>_<YYYYMMDD>.csv`（含 sentiment / theme / persona）
...
- **输入**：U7 输出
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/analyzed/preference_3decisions_<YYYYMMDD>.csv`（3 份偏好表合一）
- **预计耗时**：3h（3 决策点 × 1h）
...
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/data/analyzed/flint2_reference_summary_<YYYYMMDD>.md`（半页）
...
- **输入**：U8 + U9 输出
- **输出产物**：`DT-2026-04-28-01_BE7200-User-Research/DT-2026-04-28-01_BE7200-User-Research/DT-2026-04-28-01_BE7200-User-Research.html`（在 ticket 根下的同名子目录"交付包"里；4 章：执行摘要 / ID 偏好 / 网口偏好 / USB 偏好，Flint2 参考组放在执行摘要内）
...
- 包含：U1 / U2 / U3 / U4 / U5
- 验收：4 个数据源 CSV 落盘在 `data/raw/<source>/`；场景词典 v1 落盘在 `data/analyzed/`

### M2：标注 + 语义分析完成（≈ 启动后 10–12h）
- 包含：U6 / U7
- 验收：`labeled_*.csv` 与 `llm_*.csv` 落盘在 `data/analyzed/`；100 条人工 vs LLM 一致性 ≥ 80%

### M3：统计完成（≈ 启动后 13–14h）
- 包含：U8 / U9
- 验收：3 份决策点偏好表 + Flint2 半页摘要均落 `data/analyzed/`

### M4：HTML 报告交付（≈ 启动后 19–20h）
- 包含：U10
- 验收：`DT-2026-04-28-01_BE7200-User-Research.html` 落 ticket 根下同名子目录（交付包）；浏览器渲染正常；4 章完整；与 alignment_v2 第 6 章 MVP 承诺对齐
...
- **路径全在 ticket 单根下**：`DT-2026-04-28-01_BE7200-User-Research/data/raw/<source>/...` 与 `data/analyzed/...`，与 ticket-aligner / html-report / delivery-message 等同根；ticket 完整可独立打包归档。
