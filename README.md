...
- **`/social-reviews-analyzer`**：scratch 中间产物（`units.jsonl` / `analyses.jsonl`）放在 `data/scratch/`（建议加进 `.gitignore`），最终样本 CSV 落 `data/analyzed/<merged>.csv`
- 其他自定义数据脚本：原始数据落 `data/raw/<source>/`、分析产物落 `data/analyzed/`
...
│   └── delivery.md
├── data/                                                  ← 数据资产（给机器分析）
│   ├── raw/<source>/<descriptor>_<YYYYMMDD>.<ext>
│   ├── analyzed/<descriptor>_<YYYYMMDD>.<ext>
│   └── scratch/                                           ← gitignored（中间产物）
└── 66_Manual-SFT1200-Translation-FR-DE-ES-PL/             ← 交付包（同名子目录）
    └── 66_Manual-SFT1200-Translation-FR-DE-ES-PL.html
```

整个目录可以独立打包归档；要对外发时，只需 zip **同名子目录**（不会把 `docs/` 过程材料和 `data/` 分析数据误打进去）。

### 设计哲学

- **过程 / 数据 / 交付 三层用文件位置区分**：`docs/` = 给人读的过程材料、`data/` = 给机器分析的数据资产、`<编号>_<slug>/`（同名子目录）= 对外可发的交付包；要交付时 zip 同名子目录即可
